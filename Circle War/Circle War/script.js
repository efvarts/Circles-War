...
const health = (x,y,currentHealth,maxHealth) => {
    // Normalize health values
    currentHealth /= 2;

    // Health background
    ctx.fillStyle = "red";
    ctx.fillRect(x - 24, y - 29, 48, 8);

    // Health display
    ctx.fillStyle = "#34eb40";
    ctx.fillRect(x - 25, y - 30, currentHealth, 10);
};

health(800,200,r(100),100);
...
var l=setInterval(function(){
    ...
    var z = setInterval(function() {
        ...
        for (let i = 0; i < iter; i++) {
            health(arr[i].x, arr[i].y, arr[i].health, 100);
        }
    },50);
